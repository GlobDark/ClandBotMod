...
const clientInvRegex = /(https?:\/\/)?(www\.|canary\.|ptb\.)?discord(app)\.com\/(api\/)?oauth2\/authorize\?([^ ]+)\/?/gi;
...
module.exports = {
  ...
  stripInvites(str, { guild = true, client = true, text = '[redacted invite]' } = {}) {
    if (guild) str = str.replace(inviteRegex, text);
    if (client) str = str.replace(clientInvRegex, text);
    return str;
  },
